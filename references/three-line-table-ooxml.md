...
除此之外**全部设为无**：所有竖线（left/right/insideV）、表体各行之间的横线（insideH）都不画；也不要在行级 `w:tblPrEx/w:tblBorders` 或表体单元格的 `w:tcBorders` 上逐行保留 `top`/`bottom` 横线，否则 Word/PDF 里仍会显示成横向网格。
...
- 审计脚本：`TABLE_BORDERS`（竖线/内部网格，含表格样式从 styles.xml 带来的网格和 `tblPrEx` 行级例外网格）、`TABLE_SHADING`（底纹）、`TABLE_RULES`（顶/底线缺失、行间横线、`tblPrEx` 行级例外横线、表体单元格逐行底边线、中间线不比上下线细）都是 **FAIL 级**、直接拦截交付；最终在 MS Word 中打开目视复核。
