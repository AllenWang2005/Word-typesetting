...
- Tables: white three-line tables — the top and bottom rules visible and **thicker** (~1.5 pt, `w:sz≈12`), a single **thinner** header rule in the middle (~0.75 pt, `w:sz≈6`), and every other border (verticals, inside horizontals) set to none. **Every cell must be shading-free (white)**: remove the table style reference (or use a plain style), zero the `w:tblLook` conditional-formatting flags, and clear every `w:shd` — header shading usually comes from the table style's `firstRow` conditional format, not from a manual fill. Do not use a gridded or header-shaded table style; set the borders explicitly (see `references/three-line-table-ooxml.md`). A "no top/bottom rule, thick middle line" result is wrong. Ordinary table text is 五号/10.5 pt preferred (小四 tolerated only for ordinary non-formula text), but **in-table OMML formulas and quantity symbols must be 五号 exactly** (`w:sz=21`, `w:szCs=21`); body formulas remain 小四 (`w:sz=24`). Table content is centered by default; units go in the header. The table caption goes **above** the table, centered. Repeat the header row across page breaks; do not split a table mid-row. Normalize existing tables that do not meet the standard.
...
- Table text is larger than the body or not the expected 五号/10.5 pt (小四/12 pt tolerated only for ordinary non-formula table text), or an in-table OMML formula/symbol is not explicitly 五号 (`w:sz=21`, `w:szCs=21`), unless the user or official template explicitly requires a different size.
