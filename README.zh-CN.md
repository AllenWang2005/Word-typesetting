...
**字体排版与语言**
- 中文正文宋体/SimSun,英文与数字 Times New Roman;一、二级标题黑体(不加粗),三级标题宋体加粗;标题样式在 `styles.xml` 中显式写入字体,避免 WPS/Word 继承错字体。
...
- 公式用 LaTeX 编写并渲染为**原生 Word OMML**——变量斜体,单位/运算符/函数正体;公式编号按章右对齐,正文写"由式 (3-1) 可得";WPS 下可用无边框 1×3 公式布局表,它不按普通三线表审计。
...
| `HEADING_FONT` / `BODY_FONT` | FAIL/WARN | 标题解析到错误字体 / 正文用了黑体 |
